...
class PurchaseFlow
{
    ...
    protected function calculateTotal(ItemHolderInterface $itemHolder)
    {
        ...
        if ($itemHolder instanceof Order) {
            ...
            $total = $itemHolder->getSubTotal() + $itemHolder->getDeliveryFeeTotal();
    ...
    protected function calculateDeliveryFeeTotal(ItemHolderInterface $itemHolder)
    {
        ...
        if ($itemHolder instanceof Cart) {
            $total = $itemHolder->getItems()
                ...
                ->reduce(function ($sum, ItemInterface $item) {
                    ...
                    // $sum += $item->getPriceIncTax() * ceil($item->getQuantity() / $item->getShipCount());
        ...
        if ($itemHolder instanceof Order) {
            foreach($itemHolder->getMergedProductOrderItems() as $OrderItem){
                ...
                $ship_count = $Product->getShipCount();
                if ($ship_count == 0) $ship_count = 1;
                $total += ceil($OrderItem->getQuantity() / $ship_count) * $ProductClass->getDeliveryFee();
            }
        }
