...
class EditController extends AbstractController
{
     ...
     * @Route("/%eccube_admin_route%/order/{id}/edit/{page_no}", requirements={"id" = "\d+", "page_no" = "\d+" }, name="admin_order_edit_pagination")
     * @Template("@admin/Order/edit.twig")
     */
    public function index(Request $request, $id = null, $page_no = 1, RouterInterface $router, Paginator $paginator )
    {
        $TargetOrder = null;
        $OriginOrder = null;
        ...
        if(count($text_array) > 1) {
            $is_bulk = false;
            foreach($text_array as $key => $item) {
                if($key == count($text_array) - 1) break;
                $temp = explode(',', $item);
                $i = 0;
                foreach($temp as $t) {
                    if($i == 0)                
                    {
                        $Prize = $PrizesPerProductRepo->find($t);
                    }
                    if($i == 1){
                        $p_open = $t;
                    }
                ...
                if(strpos($Prize->getPrizeName(), '###') !== false && $Prize->getPrizeColor()) {
                    if (!$is_bulk) {
                        array_push($result, array($Prize->getPrizeGrade(), $Prize->getPrizeName(), intval($Prize->getPrizeColor())));
                        $is_bulk = true;
                    }
                    continue;
                }
                array_push($result, array($Prize->getPrizeGrade(), $Prize->getPrizeName(), $p_open));
                $is_bulk = false;
            }
        }
