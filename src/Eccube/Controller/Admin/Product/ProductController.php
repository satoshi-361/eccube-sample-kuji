...
use Plugin\LotteryProbability\Entity\Config as LotteryConfig;
...
class ProductController extends AbstractController
{
	...
	/**
     * @var ProductAssistRepository
     */
	protected $productAssistRepo;

	/**
     * @var ProductAssistConfigRepository
     */
	protected $productAssistConfigRepo;
	 ...
	 * @param LotteryProbabilityRepository $lotteryRepository
	 * @param ProductAssistRepository $productAssistRepo
	 * @param ProductAssistConfigRepository $productAssistConfigRepo
     */
    public function __construct(
		...
		LotteryProbabilityRepository $lotteryRepository,
		ProductAssistRepository $productAssistRepo,
		ProductAssistConfigRepository $productAssistConfigRepo
    ) {
		...
		$this->productAssistRepo = $productAssistRepo;
		$this->productAssistConfigRepo = $productAssistConfigRepo;
    }
    ...
    public function index(Request $request, $page_no = null, Paginator $paginator)
    {
        ...
        $qb = $this->getDoctrine()->getRepository(Product::class)->findBy([], ['position' => 'asc']);
    ...
    /**
     * @Route("/%eccube_admin_route%/product/product/image/anim", name="admin_product_image_anim", methods={"POST"})
     */
    public function registerAnimImage(Request $request)
    {
        $id = $request->get('id');
        $image = $request->get('image');

        if (!is_null($image)){
            $animImage = new ProductAnimImage();

            $animImage->setAnimImage($image);
            $animImage->productId = $id;

            $this->entityManager->persist($animImage);
            $this->entityManager->flush();
        }

        return $this->json(['count' => count($this->getDoctrine()->getRepository(ProductAnimImage::class)->findAll())], 200);
    }
    ...
    public function edit(Request $request, $id = null, RouterInterface $router, CacheUtil $cacheUtil)
    {
        ...
        if (is_null($id)) {
            ...
            $lotteryProb = $this->lotteryRepository->newConfig();
            $lotteryProb->setProductID($Product);
            $lotteryProbs = array($lotteryProb);
			
			$productAssist = new ProductAssist();
			$productAssist->product_id = $this->getDoctrine()->getRepository(Product::class)->findBy(array(),array('id'=>'DESC'),1,0)[0]->getId()+1;
			$Product->product_assist_id = $this->getDoctrine()->getRepository(ProductAssist::class)->findBy(array(),array('id'=>'DESC'),1,0)[0]->getId() + 1;
			...
			$productAssistConfs = array($productAssistConfig);
            ...
            $lotteryProbs = $this->lotteryRepository->findBy(['product_id' => $id]);
            if (empty($lotteryProbs)){	
                $lotteryProb = $this->lotteryRepository->newConfig();
                $lotteryProb->setProductID($Product);
                $lotteryProbs = array($lotteryProb);
            }
            ...
            // $animImage = $this->getDoctrine()->getRepository(ProductAnimImage::class)->findBy(['productId' => $Product->getId()], ['id' => 'DESC']);
            // if(null !== $animImage && count($animImage) > 0 ){
            //     if(count($animImage) > 0) $animImage = $animImage[0];
            // }


			$productAssist = $this->productAssistRepo->find($Product->product_assist_id);
			if (empty($productAssist)){
				$productAssist = new ProductAssist();
				$productAssist->product_id = $Product->getId();
				if(empty($this->productAssistRepo->findAll()))
					$Product->product_assist_id = 1;
				else {
					$productAssistLastId = $this->productAssistRepo->findOneBy([], ['id' => 'DESC'])->getId() + 1;
					$Product->product_assist_id = $productAssistLastId;
				}
			}
        }
...
//        $lotteryProbs = $this->lotteryRepository->findBy(['product_id' => $id]);
//        $form['lottery_probability']->setData($lotteryProbs);
        $form['product_assist']->setData($productAssist);
        $productAssistConfig = new ProductAssistConfig();   
		...
		foreach($productAssist->getSettings() as $item)
		{
			if($item->getGroupId() == 1)
				array_push($productAssistBulks, $item);
			else if($item->getGroupId() == 2)
				array_push($productAssistConfirmed, $item);
			else if($item->getGroupId() == 3)
				array_push($productAssistLottery, $item);
		}			
        $form['product_assist']['bulk_setting']->setData($productAssistBulks);
        $form['product_assist']['confirmed_lottery']->setData($productAssistConfirmed);
        $form['product_assist']['lottery_probability']->setData($productAssistLottery);
...
//            if ($form->isValid()) {
                ...
                $lotteryProbs = $form->get('lottery_probability')->getData();
                foreach($lotteryProbs as $lotteryProb)
                {
                    $this->entityManager->persist($lotteryProb);
                    $this->entityManager->flush();         
                }
                ...
                if(count($delete_images)) {
                    $delete_images = $Product->getProductImage();
                    foreach ($delete_images as $delete_image) {
                        $ProductImage = $this->productImageRepository
                            ->findOneBy(['file_name' => $delete_image->getFileName()]);
                        ...
                        // 削除
                        $fs = new Filesystem();
                        $fs->remove($this->eccubeConfig['eccube_save_image_dir'].'/'.$delete_image->getFileName());
                    }
                }
                ...
                foreach($productAssist->getSettings() as $item) {
                    $productAssist->getSettings()->removeElement($item);
                    $item->setProductAssist(null);
					$this->entityManager->persist($item);
                }
                $this->entityManager->persist($productAssist);
                $this->entityManager->flush();

                $animate_image = $form['animate_image']->getData();
                $Product->setAnimateImage($animate_image);

				$productAssist = $form['product_assist']->getData();
				
				$productAssistBulks = $form['product_assist']['bulk_setting']->getData();
				foreach($productAssistBulks as $item){
					$productAssist->addSetting($item);
					$item->setProductAssist($productAssist);
					$item->setGroupId(1);
					$this->entityManager->persist($item);
				}
				
				$productAssistConfirmed = $form['product_assist']['confirmed_lottery']->getData();
				foreach($productAssistConfirmed as $item){
					$productAssist->addSetting($item);
					$item->setProductAssist($productAssist);
					$item->setGroupId(2);
					$this->entityManager->persist($item);
				...
				$productAssistLottery = $form['product_assist']['lottery_probability']->getData();
				foreach($productAssistLottery as $item){
					$productAssist->addSetting($item);
					$item->setProductAssist($productAssist);
					$item->setGroupId(3);
					$this->entityManager->persist($item);
				...
				$Product->product_assist_id = $productAssist->getId();
				$productAssist->product_id = $Product->getId();
...
//            }
        ...
        $prizeProducts = $this->prizeListRepository->findAll();
		$lastProduct = $this->productRepository->findBy(array(),array('id'=>'DESC'),1,0);
        if ($Product->getId())
            $currentID = $Product->getId();
        else $currentID = $lastProduct[0]->getId() + 1;

        // if(isset($animImage) && null !== $animImage && !is_array($animImage)){
        //     $animImage = $animImage->getAnimImage();
        // } 
        // else $animImage = ''; 
    ...
    /**
     * @Route("/%eccube_admin_route%/product/{id}/register/category",  name="admin_product_register_category", requirements={"id" = "\d+"})
     *
     * @param Request $request
     *
    */ 
    public function registerNewCategry(Request $request, $id = null)
    {
        $Product = $this->productRepository->find($id);

        $Product->premium = $request->get('premium');
        $Product->niconico = $request->get('niconico');
        $Product->specifics = $request->get('specific');
        $Product->limit_count = $request->get('limit');
        
        dump($Product->premium);
        dump($Product->niconico);
        dump($Product->specifics);
        dump($Product->limit_count);
    ...
    */
    public function sortAction(Request $request)
    {
        ...
        $sort_info = json_decode($request->get('sort_info'));

        foreach($sort_info as $key => $value) {
            $Product = $productRepository->findOneBy(['id' => $value]);
            $Product->setPosition($key + 1);
            $em->persist($Product);
        }
        $em->flush();
