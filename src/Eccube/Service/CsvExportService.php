...
class CsvExportService
{
     ...
     */
    public function exportHeader($prize_count = 0)
    {
    ...
    public function getData(Csv $Csv, $entity, $custom_entity_name = null)
    {
        ...
        if(gettype($entity) != 'object'){
            if($csvEntityName == $custom_entity_name || $custom_entity_name == 'prize_info')
                return $entity;
            return null;
        }
        ...
        if ($data instanceof \Eccube\Entity\AbstractEntity) {
            if (EntityUtil::isNotEmpty($data)) {
                // if($data instanceof Plugin\PrizesPerProduct\Entity\Config) return 'hello';
